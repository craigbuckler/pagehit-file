...
module.exports = class {
  ...
  constructor() {
    ...
    (async () => {
      ...
      this.folder = await file.folderUsable(dataFolder);
      if (!this.folder) return;

      // fetch all JSON files sorted most recent first
      this.saved = await file.folderList(this.folder.path, 'json');
  ...
  async save() {
    ...
    if (await file.write(fn, JSON.stringify(this.counter))) {

      // delete old files
      file.unlinkMany(this.saved);
